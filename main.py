...
def horovod():
    ...
    writer = SummaryWriter(f'runs/{hvd.local_rank()}')
    ...
    for epoch in range(100):
        ...
        train_loss, train_acc = engine.bert.train(
                net=net,
                optimizer=optimizer,
                criterion=criterion,
        ...
        writer.add_scalar('data/valid_acc', valid_acc, epoch)
        writer.add_scalar('data/lr', scheduler.get_lr()[0], epoch)

        scheduler.step()

        if hvd.local_rank() == 0:
            torch.save(net.state_dict(), f'saved/bert.pt')
